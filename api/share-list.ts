...
export async function handler(req: IncomingMessage, res: ServerResponse) {
    ...
    try {
        ...
        if (!BREVO_API_KEY) {
            ...
            res.end(JSON.stringify({ error: 'BREVO_API_KEY is not configured' }));
            return;
        }
        ...
        if (!Array.isArray(recipientEmails) || recipientEmails.length === 0) {
            res.writeHead(400, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ error: 'recipientEmails must be a non-empty array' }));
            return;
        }
        ...
        res.end(JSON.stringify({
            message: `Emails enviados a ${recipientEmails.length} destinatarios vía Brevo.`,
